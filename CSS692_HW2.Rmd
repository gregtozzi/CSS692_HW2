...
The modularity associated with the grouping assigned by the investigators can be computed by first treating the graph as undirected by applying the `as.undirected` function with `mode = collapse`, then applying the `modularity` function to the graph using the given groupings.  The resulting modularity is `r modularity(as.undirected(Attiro, mode = 'collapse'), Group) %>% round(4)` which is essentially no better than that of an undivided network.

#### Hand-checking igraph's modularity calculations

The extremely low value of modularatiy for the graph using the investigators' partitioning made me question how igraph is computing modularity.  As a check, I implimented a modularity computation based on the matrix formulation available on Wikipedia.  My implimentation is available on GitHub 
